...
// upload a custom-format dump, optionally taking a safety dump first.
...
func (s *Server) restoreSubmit(w http.ResponseWriter, r *http.Request) {
	...
	if len(fhs) != 1 {
		redirectTo(w, r, "/restore", "", "attach exactly one .dump file")
		return
	}
	if fhs[0].Size == 0 {
	...
	defer src.Close()
	_, filename, err := s.engine.StartImport(dbe, src, backupFirst)
	if err != nil {
